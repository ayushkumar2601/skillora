...
export default function AdminDashboardClient({ user, stats, departmentStats, atRiskStudents }: AdminDashboardClientProps) {
  ...
  const deptData = departmentStats.map((dept, index) => ({
    ...
    value: dept.total_students,
    color: ['hsl(var(--primary))', 'hsl(var(--secondary))', '#f87171', '#fbbf24', '#34d399'][index % 5],
  }))
      ...
      <main className="lg:pl-64 pt-16 min-h-screen relative overflow-hidden">
        ...
        <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            {[
              { label: 'Total Students', value: stats.totalStudents.toString(), icon: Users, color: 'text-primary', bg: 'bg-primary/10' },
              { label: 'Avg. Attendance', value: `${stats.avgAttendance}%`, icon: BarChart3, color: 'text-secondary', bg: 'bg-secondary/10' },
              { label: 'Placement Rate', value: stats.placementRate, icon: Briefcase, color: 'text-green-600', bg: 'bg-green-500/10' },
              { label: 'Risk Alerts', value: stats.riskAlerts.toString(), icon: AlertTriangle, color: 'text-destructive', bg: 'bg-destructive/10' },
            ].map((stat, i) => (
            ...
            <Card className="glass-card p-8 flex flex-col items-center justify-center text-center">
              <div className="relative w-full h-[250px] mb-8">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <p className="text-3xl font-black text-slate-900 leading-none">{stats.totalStudents}</p>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <Card className="glass-card border-l-4 border-l-destructive overflow-hidden">
              <div className="p-6 border-b border-border flex items-center justify-between">
                ...
                <Badge variant="destructive" className="font-bold">{atRiskStudents.length} ALERTS</Badge>
              </div>
              <div className="p-2">
                {atRiskStudents.slice(0, 3).map((student, i) => (
                  <div key={i} className="p-4 flex items-center justify-between hover:bg-red-50/50 rounded-xl transition-colors group cursor-pointer">
                    <div>
                      <h4 className="font-bold text-slate-800 group-hover:text-slate-900">{student.full_name}</h4>
                      <p className="text-xs text-muted-foreground font-medium">{student.department} - Semester {student.semester}</p>
                    </div>
                    <div className="text-right">
                      <Badge variant="outline" className="font-black text-[10px] text-red-600 border-current">
                        {student.stress_index}% STRESS
                      </Badge>
            ...
            <Card className="glass-card overflow-hidden">
              ...
              <div className="p-6 grid grid-cols-2 gap-4">
                <div className="p-5 rounded-2xl bg-primary/5 border border-primary/10 group cursor-pointer hover:bg-primary/10 transition-all">
                  ...
                  <h4 className="font-black text-slate-900 leading-tight">Avg GPA {departmentStats[0]?.avg_gpa || 'N/A'}</h4>
                  <p className="text-[10px] text-muted-foreground font-bold mt-1 uppercase tracking-tighter">Top Department</p>
                </div>
                <div className="p-5 rounded-2xl bg-secondary/5 border border-secondary/10 group cursor-pointer hover:bg-secondary/10 transition-all">
                  ...
                  <h4 className="font-black text-slate-900 leading-tight">Placement Target</h4>
                  <p className="text-[10px] text-muted-foreground font-bold mt-1 uppercase tracking-tighter">{stats.placementRate} Ready</p>
                </div>
              </div>
